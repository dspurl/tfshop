<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <router-link v-permission="$store.jurisdiction.SeckillCreate" :to="'SeckillCreate'">
    ...
    <el-table
      ...
      <el-table-column type="expand">
        <template slot-scope="scope">
          <el-table
            :data="scope.row.seckill_sku"
            style="width: 100%;">
            <el-table-column label="秒杀价">
              <template slot-scope="props">
                <div>{{ props.row.seckill_price | 1000 }}</div>
              </template>
            </el-table-column>
      ...
      <el-table-column label="图片" prop="name" width="120">
        <template slot-scope="scope">
          <el-image
            :src="scope.row.resources_many.find(item => item.depict === 'seckill_zimg').img"
      ...
      <el-table-column label="秒杀活动简介" prop="abstract" width="300">
        <template slot-scope="scope">
      ...
      <el-table-column label="减库存方式" prop="is_inventory" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.is_inventory }}</span>
        </template>
      </el-table-column>
      <el-table-column label="秒杀状态" prop="state">
        <template slot-scope="scope">
          <span>{{ scope.row.state }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否上架" prop="is_show">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" class-name="small-padding fixed-width" width="120" fixed="right">
        <template slot-scope="scope">
          <router-link v-permission="$store.jurisdiction.SeckillEdit" :to="{ path: 'SeckillEdit', query: { id: scope.row.id }}">
            <el-tooltip class="item" effect="dark" content="编辑" placement="top-start">
              <el-button type="primary" icon="el-icon-edit" circle/>
            </el-tooltip>
          </router-link>
          <el-tooltip v-permission="$store.jurisdiction.SeckillDestroy" class="item" effect="dark" content="删除" placement="top-start">
    ...
    <div class="pagination-operation">
      <div class="operation">
        ...
        <el-button v-permission="$store.jurisdiction.SeckillDestroy" :loading="formLoading" size="mini" type="danger" @click="handleAllDelete()">删除</el-button>
      </div>
