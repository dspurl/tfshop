	...
	<el-container>
		<el-header>
			<div class="left-panel">
				...
				<el-button v-auth="['PermissionEdit']" type="primary" plain :disabled="selection.length!=1" @click="permission">{{$t('role.permissionSetting')}}</el-button>
			</div>
			...
			<scTable ref="table" :apiObj="apiObj" :params="params" :column="column" row-key="id" @selection-change="selectionChange">
				...
				<el-table-column :label="$t('general.operation')" fixed="right" align="right" width="140">
					<template #default="scope">
						<el-button v-auth="['RoleView']" type="text" size="small" @click="table_show(scope.row, scope.$index)">{{$t('general.view')}}</el-button>
						<el-divider v-auth="['RoleView']" direction="vertical"></el-divider>
						<el-button v-auth="['RoleEdit']" type="text" size="small" @click="table_edit(scope.row, scope.$index)">{{$t('general.edit')}}</el-button>
