...
export default {
		...
		return {
			...
			column: [
				...
				},
				{
					...
					filters: [
						{
							text: this.$t("admin.form.portrait.label.already"),
							value: 1,
						},
						{
							text: this.$t("admin.form.portrait.label.no"),
							value: 0,
						},
					],
				},
				...
				},
				{
					...
					extend: {
						...
						data: [
							{
								label: this.$t(
									"admin.form.portrait.label.already"
								),
								value: 1,
							},
							{
	...
	methods: {
		...
		async table_del(row) {
			try {
				const res = await this.$API.admin.destroy.post(row.id);
				if (!res) {
					return false;
				}
		...
		async batch_del() {
			this.$confirm(
				...
				.then(async () => {
					...
					try {
						const res = await this.$API.admin.destroy.post(0, {
		...
		async getGroup() {
			...
			const res = await this.$API.role.list.get({ all: true });
			if (!res) {
				return false;
			}
			this.showGrouploading = false;
			const allNode = { id: "", introduction: this.$t("general.all") };
		...
		groupFilterNode(value, data) {
			...
			return data.introduction.indexOf(value) !== -1;
		},
		//树点击事件
		groupClick(data) {
			var params = {
