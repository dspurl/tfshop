...
export default {
    ...
    data(){
        return {
            ...
            rules: {
                ...
                pid: [
                    {required: true, message: this.$t('login.userError'), trigger: 'change'}
                ],
                type: [
                    {required: true, message: this.$t('login.userError'), trigger: 'change'}
                ],
                api: [
                    {required: true, message: this.$t('login.userError'), triggeblurr: 'blur'}
                ]
            },
            predefineColors: [
                '#ff4500',
    ...
    methods: {
        ...
        //表单注入数据
        setData(data, pid){
            // data.is_hidden = data.is_hidden === 1 ? true : false
            // data.is_hidden_breadcrumb = data.is_hidden_breadcrumb === 1 ? true : false
