...
<template>
	<el-container>
		<el-aside width="300px" v-if="$AUTH('ResourceGroup')" v-loading="menuLoading">
			<el-container>
				<el-main class="nopadding" style="min-height: 200px">
					<el-tree ref="menu" class="menu" node-key="treeProps.key" :data="menu" :props="treeProps"
						:draggable="$AUTH('ResourceGroupSort')" highlight-current :expand-on-click-node="false"
						check-strictly :show-checkbox="!isSelect" @node-click="groupClick" @node-drop="nodeDrop"
						...
						<template #default="{ node, data }">
							<span class="custom-tree-node el-tree-node__label">
								...
								<span v-if="data.id" class="do">
									<el-icon v-auths="['ResourceGroupCreate']" @click.stop="edit(node, data)">
										<el-icon-edit />
									</el-icon>
									<el-icon v-auths="['ResourceGroupCreate']" @click.stop="add(node, data)">
										<el-icon-plus />
									</el-icon>
								</span>
							</span>
				...
				<el-footer v-if="!isSelect" style="height: 51px">
					<el-button v-auths="['ResourceGroupCreate']" type="primary" size="small" icon="el-icon-plus"
						@click="add()"></el-button>
					<el-button v-auths="['ResourceGroupDestroy']" type="danger" size="small" plain icon="el-icon-delete"
						@click="delMenu"></el-button>
					<el-button @click="allClick">{{
						$t("file_select.allResources")
					}}</el-button>
				</el-footer>
			</el-container>
			...
			<el-main class="sc-file-select__files" v-loading="listLoading">
				<el-row class="sc-file-select__top">
					<el-col class="upload" v-if="!hideUpload" :lg="18">
						<el-upload class="sc-file-select__upload" action multiple :limit="limit" :show-file-list="false"
							...
							<el-button v-auths="['ResourceCreate']" type="primary" icon="el-icon-upload">{{
								$t("file_select.localUpload") }}</el-button>
						</el-upload>
						<el-button v-if="!hideEdit && !isSelect" v-auths="['ResourceGroups', 'ResourceDestroy']"
							:type="!isOperation ? 'success' : 'info'" @click="isOperation = !isOperation">{{
								isOperation ? $t("general.preview") : $t("general.edit")
							}}</el-button>
						<el-popover placement="top-start" :width="300" trigger="hover"
							:content="`上传文件大小不超过${maxSize}MB,上传格式：${accepts}`">
					...
					<el-col :lg="6" class="keyword">
						<el-input v-model="keyword" prefix-icon="el-icon-search" :placeholder="$t('file_select.search')"
							clearable @keyup.enter="search" @clear="search"></el-input>
					</el-col>
				</el-row>
				<div class="sc-file-select__list">
					<el-scrollbar ref="scrollbar">
						<el-empty v-if="fileList.length == 0 && data.length == 0" :description="$t('general.noData')"
						...
						<div v-for="(item, index) in data" :key="item[fileProps.key]" class="sc-file-select__item"
							:class="{ active: value.includes(item[fileProps.url]) }" @click="select(item)"
							v-loading="itemLoading">
							<div class="sc-file-select__item__file">
								<template v-if="isSelect || isOperation">
									<div class="sc-file-select__item__checkbox" v-if="multiple && !isSelect">
										<el-icon>
											<el-icon-check />
										</el-icon>
									</div>
								...
								<div v-else class="operation-box">
									...
									<el-popconfirm :title="$t('general.sureDelete')" @confirm="del(item, index)">
										<template #reference>
											<el-icon class="icon">
												<el-icon-delete v-auths="['ResourceDestroy']" color="#FFFFFF" />
											</el-icon>
										</template>
									</el-popconfirm>
								</div>
								<el-image v-if="_isImg(item[fileProps.url])" :src="item[fileProps.url]" fit="contain"
									lazy></el-image>
								<el-image v-else-if="_isWord(item[fileProps.url])"
									:src="require('@/assets/file/WORD.png')" fit="scale-down" lazy></el-image>
								<el-image v-else-if="_isPdf(item[fileProps.url])"
									:src="require('@/assets/file/PDF.png')" fit="scale-down" lazy></el-image>
								<el-image v-else-if="_isExcl(item[fileProps.url])"
									:src="require('@/assets/file/ECEL.png')" fit="scale-down" lazy></el-image>
								<el-image v-else-if="_isTxt(item[fileProps.url])"
									:src="require('@/assets/file/TXT.png')" fit="scale-down" lazy></el-image>
								<tf-video v-else-if="_isVideo(item[fileProps.url])"
									:sources="{ src: item[fileProps.url] }"></tf-video>
								<el-image v-else-if="_getPackage(item[fileProps.url])"
									:src="require('@/assets/file/ZIP.png')" fit="scale-down" lazy></el-image>
								<div v-else class="item-file item-file-doc">
									<i v-if="files[_getExt(item[fileProps.url])]"
										:class="files[_getExt(item[fileProps.url])].icon" :style="{
				...
				<div class="sc-file-select__do">
					<slot name="do"></slot>
					<el-button v-if="isSelect" type="primary" :disabled="value.length <= 0" @click="submit(item)">{{
						$t("general.confirm") }}</el-button>
					<template v-else>
						<template v-if="isOperation">
							<el-button v-auths="['ResourceGroups']" :loading="buttonLoading" type="primary"
								:disabled="value.length <= 0" @click="dialogVisible = true">{{ $t("file_select.group")
								}}</el-button>
							<el-popconfirm :title="$t('general.sureDelete')" @confirm="remove">
								<template #reference>
									<el-button v-auths="['ResourceDestroy']" :loading="buttonLoading" type="danger"
										:disabled="value.length <= 0">{{ $t("general.delete") }}</el-button>
								</template>
							</el-popconfirm>
						</template>
					</template>
		...
		<el-dialog :title="$t('file_select.group')" v-model="dialogVisible" :width="200" destroy-on-close
			:close-on-click-modal="false" center>
			<el-cascader v-model="groupData" :options="menu"
				:props="{ checkStrictly: true, label: 'name', value: 'id' }" clearable></el-cascader>
			<template #footer>
				<el-button @click="dialogVisible = false">{{
					$t("general.cancel")
				}}</el-button>
				<el-button type="primary" @click="saveGroup()" :loading="buttonLoading">{{ $t("general.save")
					}}</el-button>
			</template>
		</el-dialog>
	</el-container>
...
export default {
	...
	mounted() {
		this.value = this.modelValue ? this.modelValue : this.multiple ? [] : "";
		this.getMenu();
		this.getData();
		this.getAccept();
	},
	methods: {
		// 获取上传格式
		async getAccept() {
			const res = await this.$API.resourceType.detail.get({ id: this.uuid });
			let newList = res.message.map(item => {
				if (item === "gif" || item === "bmp" || item === "png" || item === "webp") {
					return `image/${item}`;
				} else if (item === "jpg" || item === "jpeg") {
					return `image/jpeg`;
				} else if (item === "mp4") {
					return `video/${item}`;
				} else if (item === "mkv") {
			...
			this.accept = newList.join(',');
			this.accepts = res.message.join(',');
		},
		//获取分类数据
		async getMenu() {
		...
		async getData() {
			...
			var reqData = {
				...
				[config.request.maxSize]: this.isSelect ? this.maxSize : '',
			};
			if (this.onlyImage) {
				reqData.type = "image";
			}
		...
		edit(node, data) {
			this.$prompt("请输入分组名称", "提示", {
				...
				.then(({ value }) => {
					this.$API.resourceGroup.edit.post(data.id, {
						name: value,
					});
					data.name = value;
		...
		select(item) {
			...
			if (this.multiple) {
				if (this.value.includes(itemUrl)) {
					...
					// 资源管理器需要选中状态才允许选择
					if ((this.isOperation || this.isSelect) && this.value.length < this.limit) {
		...
		uploadBefore(file) {
			...
			if (!maxSize) {
				this.$message.warning(
					`${this.$t("file_select.most")} ${this.maxSize}MB!`
				);
				return false;
			}
		},
