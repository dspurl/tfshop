		...
		<el-drawer :title="$t('filterBar.index.title')" v-model="drawer" :size="650" append-to-body>
			<el-container v-loading="saveLoading">
								...
								<el-main class="nopadding">
									<div class="sc-filter-main">
										...
										<div v-if="filter.length <= 0" class="nodata">
											{{ $t('filterBar.index.conditionNoData') }}
										</div>
										<table v-else>
											<colgroup>
												<col/>
											...
											<tr v-for="(item,index) in filter" :key="index">
												...
												<td>
													<py-select v-model="item.field" :options="fields" :filter="filter" :placeholder="$t('filterBar.index.field')" filterable @change="fieldChange(item)" >
													</py-select>
												</td>
												<td v-if="showOperator">
													<el-select v-model="item.operator"
															   :placeholder="$t('filterBar.index.operator')">
														<el-option
												...
												<td>
													...
													<!-- 级联选择 -->
													<el-cascader
														v-if="item.field.type == 'cascader'"
														v-model="item.value"
														:options="item.field.extend.data"
														:props="item.field.extend.props"
														:show-all-levels="item.field.extend.showAllLevels"
														filterable
														:placeholder="item.field.extend.placeholder">
													</el-cascader>
													<!-- 日期 -->
													<el-date-picker
													...
													<!-- 月份 -->
													<el-date-picker
														v-if="item.field.type == 'monthrange'"
														v-model="item.value"
														type="monthrange"
														value-format="YYYY-MM-DD"
														style="width: 100%;"
				...
				<el-footer>
					...
					>{{ $t('filterBar.index.immediately') }}
					</el-button>
					<el-button v-if="data.id" type="primary" plain @click="saveMy('my')">{{
							$t("general.save")
						}}
					</el-button>
					<el-button
	...
	computed: {
		filterObj() {
			...
			this.filter.forEach((item) => {
				// 判断值为数组时，只取数组最后一个值，用于多级联动数据
				let value = item.value
				if (typeof item.value === 'object') {
					item.value.forEach((item2, index) => {
						if (typeof item2 === 'object') {
							item.value[index] = item2[item2.length - 1]
						}
					})
				}
				obj[item.field.value] = this.showOperator ? `${value}${config.separator}${item.operator}` : `${value}`
			})
			return obj
		}
	},
			...
			this.fields.forEach((field) => {
				...
				if (filterValue) {
					...
					if (field.type === 'select' && field.extend.multiple) {
						value = value.split(",")
					} else if (field.type === 'daterange' || field.type === 'monthrange') {
						value = value.split(",")
					}
					this.filter.push({
						field: field,
