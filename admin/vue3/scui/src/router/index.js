...
router.beforeEach(async (to, from, next) => {
	...
	if (getToken('expires_in') && to.path !== "/login") {
        if ((new Date()).getTime() >= getToken('expires_in') - 300 * 1000) { // token失效前5分钟会自动刷新token
			try {
				const refreshToken = await api.auth.refreshToken.post({refresh_token: getToken('refresh_token')})
				setToken('access_token', refreshToken.message.access_token)
				setToken('expires_in', (new Date()).getTime() + refreshToken.message.expires_in * 1000)
