...
class Plugin
{
     ...
     * @return string
     */
    public function autoGeneratePlugin($name, $api)
    {
        $this->autoElement($name, $api);
    }
  ...
  // 插件列表", $file_get_contents);
    ...
    protected function autoElement($name, $api)
    {
        ...
        $metadata = str_replace("前台插件列表", $name . "
        Route::get('$api', '" . $api . "Controller@index')->middleware(['lpower:" . ucwords($api) . "List']);    //" . $name . "列表
        Route::post('$api', '" . $api . "Controller@store')->middleware(['lpower:" . ucwords($api) . "Create']);    //" . $name . "添加保存
        Route::put('$api/{id}', '" . $api . "Controller@update')->middleware(['lpower:" . ucwords($api) . "Update']);    //" . $name . "编辑保存
        Route::delete('$api/{id}', '" . $api . "Controller@destroy')->middleware(['lpower:" . ucwords($api) . "Delete']);    //" . $name . "删除
        //前台插件列表     
