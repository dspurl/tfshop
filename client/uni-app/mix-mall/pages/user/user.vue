	...
	<view class="container">
		...
		<view class="cover-container" :style="[{
			...
			<view class="order-section">
				<view class="order-item" @click="navTo('/pages/indent/list?state=0')" hover-class="common-hover"
					:hover-stay-time="50">
					<text class="yticon icon-shouye"><text v-if="quantity.all" class="cu-tag badge">{{ quantity.all
							}}</text></text>
					<text>{{ $t('good_indent.state.all') }}</text>
				</view>
				<view class="order-item" @click="navTo('/pages/indent/list?state=1')" hover-class="common-hover"
					:hover-stay-time="50">
					<text class="yticon icon-daifukuan"><text v-if="quantity.obligation" class="cu-tag badge">{{
						quantity.obligation }}</text></text>
					<text>{{ $t('good_indent.state.pay') }}</text>
				</view>
				<view class="order-item" @click="navTo('/pages/indent/list?state=2')" hover-class="common-hover"
					:hover-stay-time="50">
					<text class="yticon icon-gouwuche_"><text v-if="quantity.waitdeliver" class="cu-tag badge">{{
						quantity.waitdeliver }}</text></text>
					<text>{{ $t('good_indent.state.deliver') }}</text>
				</view>
				<view class="order-item" @click="navTo('/pages/indent/list?state=3')" hover-class="common-hover"
					:hover-stay-time="50">
					<text class="yticon icon-yishouhuo"><text v-if="quantity.waitforreceiving" class="cu-tag badge">{{
						quantity.waitforreceiving }}</text></text>
			...
			<view class="history-section icon">
				...
				<list-cell icon="icon-dizhi" iconColor="#5fcda2" :title="$t('user.site')"
					@eventClick="navTo('/pages/address/address')"></list-cell>
				<list-cell icon="icon-shoucang_xuanzhongzhuangtai" :title="$t('user.collect')" iconColor="#54b4ef"
					@eventClick="navTo('/pages/user/collect')"></list-cell>
				<list-cell icon="icon-comment" iconColor="#e07472" :title="$t('user.inform')"
					:tips="noticeNumber ? noticeNumber : null" @eventClick="navTo('/pages/notice/notice')"></list-cell>
				<list-cell icon="icon-shezhi1" iconColor="#e07472" :title="$t('user.set')"
					@eventClick="navTo('/pages/set/set')"></list-cell>
				<list-cell cuIcon="cuIcon-global" iconColor="#e07472" :title="$t('set.multilingual')"
					@eventClick="modalName = 'multilingualModal'"></list-cell>
			</view>
			<view style="padding-top: 40rpx; text-align: center; color: #c4c3c3">TFSHOP提供技术支持</view>
			<view style="padding-top: 10rpx; text-align: center; color: #c4c3c3;font-size:22rpx;">
				TFSHOP基于MIT发行，允许任何人下载使用，请自行分辨平台合规性！</view>
		</view>
