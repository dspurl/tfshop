        ...
        <div class="list">
          <div class="li" v-for="(item,index) in list" :key="index">
            <div class="top" :class="{on: !item.state}">
              ...
              <div class="money" v-else><span>¥</span>{{ item.coupon.cost/100 }}</div>
            ...
            <div class="explain">
              <div class="dl"><div class="dt">使用条件:</div>{{ item.coupon.name }}</div>
              <div class="dl" v-if="item.coupon.end_time"><div class="dt">有效时间:</div>{{item.created_at | moment('YYYY.MM.DD')}} 至 {{item.coupon.end_time | moment('YYYY.MM.DD')}}</div>
              <div class="dl" v-else><div class="dt">有效时间:</div>不限</div>
